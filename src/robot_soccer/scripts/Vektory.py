...
import velchangers
import HomeRobot
from HomeRobot import RobotCommand
from Ball import *
from param import *
#from utilities.kick import kick
...
class Vektory:
  ...
  def sendCommand(self, vel_x, vel_y, omega, theta = 0.0):
    command = RobotCommand(-1,vel_x, vel_y, omega, theta)
    command.execute()
    sendCommand = rospy.ServiceProxy('commandSent', commandsent)
    #try:
  ...
  def go_to_point(self,x, y, lookAtPoint=None):
   ...
   #changes made on updateLocations
  ...
  def updateLocations(self):
	...
	print response
        self.locations = pickle.loads(response.pickle)
    #    print (self.locations.ball.x, self.locations.ball.y)
	#self.robotLocation = HomeRobot.RobotLocation()
        #self.robotLocation.x = self.locations.home1_x
	#self.robotLocation.y = self.locations.home1_y
   	#self.robotLocation.theta = self.locations.home1_theta
	self.robotLocation = self.locations.home1
	self.ball.point.x = self.locations.ball_x
        self.ball.point.y = self.locations.ball_y
#	self.ball.point.x = -26
#	self.ball.point.y = 87
#	self.locations.ball.x = -26
#	self.locations.ball.y = 87


	self.distanceToBall = math.sqrt((self.robotLocation.x-self.locations.ball_x)**2+(self.robotLocation.y-self.locations.ball_y)**2)
	print (self.locations.ball_x, self.locations.ball_y)
        #print 'time: %f x: %f  y: %f  theta: %f' %(robotLocation.timestamp, robotLocation.x, robotLocation.y, robotLocation.theta)
  ...
  def go(self):
    velchangers.calibrateRoboclaws()
    print "Searching for Vektor Locations Service..."
    rospy.wait_for_service('locations')
    print "Searching for Vektor Locations Update Service..."
    rospy.wait_for_service('commandSent')
