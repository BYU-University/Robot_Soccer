...
import math
import copy
#from motor_control.param import *
...
class HomeRobot:
  ...
  def getLocationFromHistory(self, stopTime = -1):
    ...
    for command in self.commandQueue:
      while 0<command.expiration:#location.timestamp < command.expiration:
...
#      location.timestamp = location.timestamp + 1
 ...
 #       return 
    ...
    if len(self.commandQueue) == 0:
      ...
      location = self.getLocation(0)#(command.timestamp=0)
      self.pendBusy()
      self.predictedLocation = location
      self.commandQueue[-1].expiration = command.timestamp=0
      self.commandQueue.append(command)
      self.postBusy()
...
#      raise HomeError('Tried to update with location before beginning of history')
    ...
    while len(self.commandQueue) > 0:
      ...
      if commandIsKill or (expiration > 0 and expiration <= 0): #newLocation.timestamp):
